...
AutoLayout.prototype._buildTreeBreadFirstSearch = function (rootFlowElement) {
  ...
  while (queue.length !== 0) {
    ...
    if (
      elementOrConnection.$type !== "bpmn:SequenceFlow" &&
  ...
  */
  ...
  if (flowElement.flowElements) {
    ...
    var childNodes = [...subprocessNode.children].filter((child) => {
      return !subprocessStartNodes.find(
        (startNode) => startNode.id === child.id
      );
    });
  ...
  oldParents.forEach((parent) => {
    ...
    while (padParent._getLevel() < maxParentLevel) {
      ...
      var message =
        ...
        " between " +
        oldParent.dsc +
        (oldParent.dsc === PADDING_NODE ? oldParent.id : "") +
        " and " +
        node.dsc +
        (node.dsc === PADDING_NODE ? node.id : "");
      console.log(message);
...
AutoLayout.prototype._layoutTreeBreadFirstSearch = function (
  parentFlowElement,
  rootDi
) {
...
AutoLayout.prototype._groupElement = function (
  elementOrConnection,
  group,
  parentDi
) {
...
AutoLayout.prototype._layoutElements = function (group, parentDi) {
  ...
  elements.forEach(function (element) {
    ...
    if (element.flowElements && element.$type === "bpmn:SubProcess") {
      ...
      element.flowElements.forEach((el) => {
        el.marked = false;
      });
      var offsetX =
        element.bounds.x +
...
function getOutgoingConnection(source, flowElements) {
  return flowElements
    ? flowElements.filter((e) => {
        return (
          is(e.$type, "bpmn:SequenceFlow") &&
          e.get("sourceRef").id === source.id
        );
      })
    : [];
}
