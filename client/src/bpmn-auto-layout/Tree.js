...
TreeNode.prototype._getChildrenCenter = function (tree) {
  ...
  return (
    first.prelim + (last.prelim - first.prelim + tree._getNodeSize(last)) / 2
  );
};
...
Tree._apportion = function (tree, node, level) {
  ...
  var j = 1;
  for (
    var k = tree.config.iMaxDepth - level;
